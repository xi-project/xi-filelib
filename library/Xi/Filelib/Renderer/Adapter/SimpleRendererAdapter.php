...
class SimpleRendererAdapter implements AcceleratedRendererAdapter
{
    ...
    {
        $statusText = self::getStatusText($response->getStatusCode());

        header("HTTP/1.1 {$response->getStatusCode()} {$statusText}");
    ...
    private static function getStatusText($statusCode)
    {
